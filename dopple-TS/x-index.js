...
app.get("/", function(req, res) {
  ...
  if (playerArray[0] != undefined) {
    ...
    if (lockPlayerCheckBox === true && resetPressed === false) {
      // Answer button pressed, checkbox CHECKED
      //console.log("Answer button pressed and checkbox checked (players locked!)");
      playerIsLocked = 1;
    } else {
      // Answer button pressed, checkbox NOT checked
      playerIsLocked = 0;
    }
  } else {
  ...
  if (playerIsLocked === 1) {
    ...
    if (playerIsLocked === 0) {
      // Since players not locked, make sure new players chosen are not same as last round.
      ...
      //if(typeof playerArray[0].winner != "undefined"){
      if (
        typeof playerArray[0].winner != "undefined" &&
        playerOne.toString() === playerArray[0].winner.charAt(0)
      ) {
        console.log(
          "New players are the same as old players!  Choosing different..."
        );
...
app.post("/node-dopple-main", function(req, res) {
  ...
  winnerLoserObject = {
    ...
    loserName: loserName,
    winnerOldScore: winnerOldScore,
    loserOldScore: loserOldScore,
    winnerELO: winnerELO,
    loserELO: loserELO,
    winnerNewScore: winnerNewScore,
    loserNewScore: loserNewScore,
